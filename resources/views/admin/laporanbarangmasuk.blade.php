...
<div class="col-lg-12">
    ...
    <div class="card mb-4">
      <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        ...
        <form action="{{ route('laporanmsuksearch') }}" method="get">
      ...
      <div class="table-responsive p-3">
        <table class="table align-items-center table-flush table-hover" id="dataTableHover">
          ...
          <tbody>
            @foreach ($data as $Lbarangmasuk)
              <tr>
                <td>{{ $Lbarangmasuk->daftaritem->kode_barang }}</td>
                <td>{{ $Lbarangmasuk->daftaritem->nama_barang }}</td>
                <td>{{ $Lbarangmasuk->qty }}</td>
                <td>{{ $Lbarangmasuk->satuan }}</td>
                <td>{{ $Lbarangmasuk->tanggal_masuk }}</td>
                <td>{{ $Lbarangmasuk->tanggal_exp }}</td>
              </tr>
            @endforeach
          </tbody>
        </table>
